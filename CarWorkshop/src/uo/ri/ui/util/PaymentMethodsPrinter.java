 ...
 */
public class PaymentMethodsPrinter extends AbstractPrinter {
	...
	for (int i = 0; i < listMaps.size(); i++) {
	    ...
	    it.remove();
	    Iterator<Entry<String, Object>> it2 = listMaps.get(i).entrySet()
		    .iterator();
	    if (tipo != "") {
		while (it2.hasNext()) {
		    Map.Entry<String, Object> pair2 = (Map.Entry<String, Object>) it2
			    .next();
		    if (pair2.getKey().length() > tipo.length()
			    && pair2.getKey().substring(0, tipo.length())
				    .equals(tipo)) {
			aux += pair2.getKey().substring(tipo.length()) + ": "
				+ pair2.getValue() + " ";
		    }
		}
	    }
